...
class SshProfileViewModel @Inject constructor(
    ...
    fun save(profile: SshProfile?, label: String, host: String, port: Int, username: String, password: String) {
        viewModelScope.launch {
            val credKey = profile?.credentialKey ?: "ssh_${UUID.randomUUID()}"
            if (password.isNotEmpty()) credentialStore.putString(credKey, password)
